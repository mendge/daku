...
// 给仅有Meta data 的dag配置日志\步骤集\handlers\邮件等运行时需要的对象依赖
...
// 将dagDef中解析出各种定时表达式的string 给dag的对应种类表达式切片
...
func buildEnvs(def *configDefinition, d, base *DAG, options BuildDAGOptions) (err error) {
	...
	if err == nil {
		...
		if base != nil {
			// options和def中没有,但是baseDag中有的环境变量,也赋给dag
...
func parseParameters(value string, eval bool, options BuildDAGOptions) (params []string, envs []string, err error) {
...
// 从options和strVariables(可能多种类型:map[k]v,[]map[k]v)中先解析出*envVariable列表,再以map的形式返回所有的环境变量
func loadVariables(strVariables interface{}, options BuildDAGOptions) (map[string]string, error) {
...
// 一种定时表达式可能是一个也可能是多个组成的列表,从map中解析出定时表达式的string给各个定时种类(启动,结束,重启)
