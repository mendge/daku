...
// Multiple JSON data can be stored in a single file and each data is separated by newline.
...
// 给jsondb.Store配置file对应的jsondb.writer
...
func ParseFile(file string) (*model.Status, error) {
	...
	// 由于运行时的状态更新以append形式,所以要读取文件的最后一个有效状态
...
// FindByRequestId finds a status file by requestId. // 返回最新状态?
...
// 返回一个新的.dat文件的 $data/dagName-md5(dagPath)/dagName.yyyyMMdd.hh.mm.ss.xxx.y.dat
