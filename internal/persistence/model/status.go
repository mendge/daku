...
func (sts *Status) MakeNodesLogPathRemote(lLogDir, rLogDir string) {
	for _, node := range sts.Nodes {
		remoteNodeLogPath := strings.Replace(node.Log, lLogDir, rLogDir, 1)
		node.Log = remoteNodeLogPath
	}
}
