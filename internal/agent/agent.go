...
// 配置reqid/graph, 检查preconditions/已经running/, 启动socket服务器, 配置.dat文件和日志文件,调用a.run
...
// 初始化当前agent, 给其的dag配置成功/失败/退出/取消等handlerOn的具体Step操作, 创建dag调度器和logManager
func (a *Agent) init() {
...
func (a *Agent) checkIsRunning() error {
	...
	if status.Status != scheduler.SchedulerStatus_None {
		return fmt.Errorf("the DAG is already running. socket=%s", a.DAG.SockAddr())
	}
	return nil
}
